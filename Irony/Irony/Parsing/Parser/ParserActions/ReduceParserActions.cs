...
namespace Irony.Parsing {
  ...
  public partial class ReduceParserAction: ParserAction {
    ...
    protected virtual ParseTreeNode GetResultNode(ParsingContext context) {
      ...
      for (int i = 0; i < childCount; i++) {
        ...
        // ## Editado
        if (childNode.IsEmptyTransient()) continue; //skip empty transient nodes
  ...
  public class ReduceTransientParserAction : ReduceParserAction {
    ...
    protected override ParseTreeNode GetResultNode(ParsingContext context) {
      ...
      for (int i = 0; i < childCount; i++) {
        ...
        //## Editado:
        if (child.IsEmptyTransient()) continue;
        return child;
      }
  ...
  public class ReduceListBuilderParserAction : ReduceParserAction {
    ...
    protected override ParseTreeNode GetResultNode(ParsingContext context) {
      ...
      // ## Modificado:
      if (listMember.IsEmptyTransient())
        return listNode;

      // ## Añadido porque necesito el delimitador también
      var delimiterMember = context.ParserStack[context.ParserStack.Count - 2];
      if (delimiterMember.Term != listNode.Term)
      {
        listNode.ChildNodes.Add(delimiterMember);
      }
      // ## Fin añadido
