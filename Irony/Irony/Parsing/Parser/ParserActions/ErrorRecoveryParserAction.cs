  ...
  public class ErrorRecoveryParserAction : ParserAction
  {
    ...
    private ParserAction FindErrorShiftActionInStack(ParsingContext context)
    {
      ...
      while (context.ParserStack.Count >= 1)
      {
        ...
        {
          /* ## Inicio: añadido para permitir recuperación de errores 
          while (context.CurrentToken != null && context.CurrentToken.Terminal != context.Parser.Language.Grammar.Eof && (context.CurrentToken.KeyTerm == null || context.CurrentToken.KeyTerm.Name != ";"))
          {
            context.Parser.ReadInput();
          }

          if (context.CurrentToken.KeyTerm != null && context.CurrentToken.KeyTerm.Name == ";")
          {
            return context.CurrentParserState.Actions.First().Value;
          }
           ## Fin: añadido para permitir recuperación de errores */
